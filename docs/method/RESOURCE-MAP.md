# Resource Map

> **Scope note.** This file is a **breadth index** — entry points across Azure, M365, migration,
> security and AI. It is deliberately shallow: most entries are documentation *roots*, which
> means "go read this set", not "this has been read".
>
> For the identity track, **[SC-300-RESOURCE-LIBRARY.md](../../SC-300-RESOURCE-LIBRARY.md) supersedes
> this file.** That one is depth-first and tiered by authority — RFCs and NIST, Microsoft,
> hands-on, rival vendors (Okta/Auth0), offensive research, tooling, practitioners, adjacent
> market — with a stated reason to read each item.
>
> ⚠ **AZ-500 retires 2026-08-31.** Its successor is **SC-500**. Study AZ-500 material for
> content; do not sit the exam.
>
> Five links in this file were dead (404) until 2026-08-09. External URLs are now gated in CI
> by `.github/workflows/coverage.yml` — a dead link fails the build.

Resources are labeled **official**, **unofficial**, or **reference**. Official documentation is the authority for current behavior, limits, pricing, and certification scope. Unofficial material is used for explanations and practical perspective, then verified against official documentation.

## Official Microsoft learning

### Certification study guides

- [AZ-104 study guide](https://learn.microsoft.com/en-ca/credentials/certifications/resources/study-guides/az-104)
- [AZ-500 study guide](https://learn.microsoft.com/en-ca/credentials/certifications/resources/study-guides/az-500)
- [SC-300 study guide](https://learn.microsoft.com/en-ca/credentials/certifications/resources/study-guides/sc-300)
- [SC-300 official study guide alias](https://aka.ms/sc300-StudyGuide)
- [SC-500 / Cloud and AI Security certification](https://learn.microsoft.com/en-us/credentials/certifications/cloud-and-ai-security-engineer-associate/)

### Official courses and plans

- [SC-300T00 course](https://learn.microsoft.com/en-ca/training/courses/sc-300t00)
- [Microsoft Learn SC-300 plan](https://learn.microsoft.com/en-ca/plans/m14ntm3n2gy41q)
- [Microsoft Learn](https://learn.microsoft.com/training/)
- [Microsoft 365 training](https://learn.microsoft.com/en-us/training/m365/)
- [Azure training](https://learn.microsoft.com/training/azure/)
- [Microsoft security training](https://learn.microsoft.com/en-us/training/browse/?subjects=security)

### Official product documentation

- [Microsoft Entra documentation](https://learn.microsoft.com/entra/)
- [Microsoft 365 documentation](https://learn.microsoft.com/microsoft-365/)
- [Exchange Online documentation](https://learn.microsoft.com/exchange/)
- [SharePoint documentation](https://learn.microsoft.com/sharepoint/)
- [Microsoft Teams documentation](https://learn.microsoft.com/microsoftteams/)
- [Microsoft Graph documentation](https://learn.microsoft.com/graph/)
- [Azure architecture center](https://learn.microsoft.com/azure/architecture/)
- [Azure Well-Architected Framework](https://learn.microsoft.com/azure/well-architected/)
- [Azure landing zones](https://learn.microsoft.com/azure/cloud-adoption-framework/ready/landing-zone/)
- [Azure migration guidance](https://learn.microsoft.com/azure/cloud-adoption-framework/migrate/)
- [Entra Connect documentation](https://learn.microsoft.com/entra/identity/hybrid/connect/)
- [Microsoft Purview documentation](https://learn.microsoft.com/purview/)
- [Microsoft Defender documentation](https://learn.microsoft.com/defender/)
- [Microsoft Sentinel documentation](https://learn.microsoft.com/azure/sentinel/)
- [Azure networking documentation](https://learn.microsoft.com/azure/networking/)
- [Azure identity and access documentation](https://learn.microsoft.com/azure/active-directory/)
- [Azure AI documentation](https://learn.microsoft.com/azure/ai-services/)
- [Azure OpenAI documentation](https://learn.microsoft.com/azure/ai-services/openai/)
- [Azure Well-Architected AI workloads](https://learn.microsoft.com/azure/well-architected/ai/)

### Official migration and deployment material

- [Microsoft 365 migration center](https://learn.microsoft.com/en-us/microsoft-365/enterprise/)
- [Exchange migration documentation](https://learn.microsoft.com/en-us/exchange/mailbox-migration/mailbox-migration)
- [SharePoint and OneDrive migration](https://learn.microsoft.com/sharepointmigration/migrate-to-sharepoint-online)
- [Teams migration and interoperability](https://learn.microsoft.com/en-us/microsoftteams/)
- [Microsoft 365 tenant-to-tenant guidance](https://learn.microsoft.com/microsoft-365/enterprise/cross-tenant-mailbox-migration)
- [Azure Migrate](https://learn.microsoft.com/azure/migrate/)
- [Azure CLI documentation](https://learn.microsoft.com/cli/azure/)
- [Azure PowerShell documentation](https://learn.microsoft.com/powershell/azure/)
- [Bicep documentation](https://learn.microsoft.com/azure/azure-resource-manager/bicep/)
- [Azure Terraform documentation](https://learn.microsoft.com/azure/developer/terraform/)

## Unofficial but useful learning material

These are supplemental explanations and practical viewpoints, not authorities. Verify commands and current product behavior against Microsoft documentation before using them.

- John Savill's Technical Training: deep Azure, identity, networking, security, and architecture explanations.
- Microsoft Mechanics: product demonstrations and feature context.
- Andy Malone / Azure Academy: Azure administration and certification-oriented walkthroughs.
- Practical 365: Microsoft 365, Exchange, Teams, and tenant administration articles.
- Tony Redmond / Office 365 for IT Pros: Microsoft 365 administration and service-change analysis.
- Quest, BitTitan, ShareGate, Cloudiway, and AvePoint technical blogs: migration patterns and product-specific workflows. Treat vendor content as product perspective, not neutral comparison.
- ITPro / CBT Nuggets / Pluralsight / LinkedIn Learning: structured labs and explanations when access is available.
- Azure Friday and Microsoft Reactor recordings: architecture and implementation demonstrations.
- AZ-104, AZ-500, SC-300, and M365 community study groups: useful for practice questions, but validate answers against official objectives and docs.

## Resource verification rule

For each important resource record:

- Date accessed.
- Product version or service state.
- Official source checked.
- Commands tested.
- Tenant/region assumptions.
- Pricing or limits verified.
- What was learned.
- What remains unverified.

Do not copy a blog's claim into a portfolio or resume without reproducing it or verifying it in official documentation.

