...
>
> **Duplication rule:** this file is the **strategy and monetization overlay**. The executable lab
> source of truth is [`SC-300-SPRINT/README.md`](SC-300-SPRINT/README.md), and the exam control tower
> is [`SC-300-EXAM-READINESS.md`](SC-300-EXAM-READINESS.md). Do not copy commands from the sprint
> into this file; link to the execution day instead.
>
...
>
> **Market baseline checked 2026-08-18:** current remote postings repeatedly ask for Entra ID,
...
> senior troubleshooting, architecture, regulated-customer delivery, automation, or multi-tenant
> consulting.
>
...
Public remote-role samples checked on 2026-08-18 show these repeated asks:

- Indeed remote Entra ID listings: hybrid identity, Entra ID, AD, SSO, MFA, Conditional Access,
  SAML/OAuth/OIDC, RBAC, app integration, PIM, access reviews, monitoring, SIEM, documentation,
  and federal or Zero Trust alignment.
- Dice remote Entra ID listings: senior consulting, Microsoft Entra ID, Active Directory, hybrid
  administration, IGA adjacency, automation, and high-end annual salary bands for senior IAM roles.
- CareerBuilder and recruiter contract samples: Microsoft Entra practitioner roles commonly list
  Conditional Access and PIM with visible contract rates around USD $60-$65/hour.
- UK hybrid/remote market samples: senior identity consultant and architect roles emphasize complex
  Microsoft 365 tenant consolidation, HLD/LLD documentation, Okta adjacency, Entra ID, Intune,
  SharePoint, Teams, OneDrive, and Azure architecture.
...
After reviewing the sprint again as a career-to-revenue plan, these are the changes that matter most:

1. **Do not market yourself as only "SC-300 in progress."** Market yourself as an identity risk
   operator who can prove tenant hardening, Conditional Access rollout safety, PIM governance,
   SaaS onboarding/offboarding, and incident response.
2. **Lead with three buyer-ready artifacts, not the full lab library.** The first money path is:
   `Tenant Risk QuickScan` → `Conditional Access Hardening` → `PIM / Access Review Evidence Pack`.
   Everything else supports those offers.
3. **Add hybrid and AD language to every interview story.** Remote job descriptions still pair
   Entra ID with AD DS, federation, sync, PKI, and legacy app troubleshooting. SC-300 is cloud-heavy,
   but senior hiring screens are hybrid-heavy.
4. **Make automation visible.** Every high-paying path improves if you can export policy state,
   compare before/after posture, and produce auditor-friendly evidence with Graph PowerShell.
5. **Use nonprofit/startup offers as wedge projects, not your ceiling.** Small fixed-scope projects
   build proof and references; enterprise remote roles still require architecture, troubleshooting,
   and operational maturity.
6. **Avoid claiming compliance certification.** Say "mapped to SOC 2 / ISO 27001 / NIST evidence"
   unless an auditor or assessor has validated the control set.
7. **Spend day 7 selling, not just studying.** A learner who passes a practice test is not yet
   monetized; a contractor with an offer menu, SOW, evidence pack, and 50 targeted outreaches has a
   pipeline.

### Priority recommendation stack

| Priority | Recommendation | Why it matters | Done when |
|---|---|---|---|
| P0 | Build and document break-glass, budget, tenant baseline, and admin role inventory first | Prevents lab lockout, surprise spend, and unverifiable claims | Baseline export and SOP exist |
| P0 | Produce a Conditional Access pack with report-only, What If, exclusions, rollback, and sign-in logs | CA is the highest interview and customer-risk topic | Policy design and test evidence exist |
| P0 | Produce a PIM/access review pack | PIM and governance turn identity work into audit evidence | Activation settings and review result screenshots/exports exist |
| P1 | Produce a workload identity / app consent pack | OAuth consent and app permissions are common breach paths and senior differentiators | App registration, consent explanation, and NHI inventory exist |
| P1 | Create one fixed-price QuickScan offer and one SOW | Converts technical work into a purchasable service | Offer menu and SOW are ready to send |
| P2 | Add Defender, Purview, and Intune adjacency proof | Remote M365 security roles bundle these with Entra | At least one control map references each area |
| P2 | Add hybrid identity troubleshooting notes | Senior roles often test AD DS + Entra sync reality | Hybrid failure-mode checklist exists |
...
Run the linked `SC-300-SPRINT/` day file for commands. Use this table to understand the business
deliverable each day must produce.
