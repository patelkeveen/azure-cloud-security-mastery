...
# --- Per-domain measured block -------------------------------------------------
# Each domain README carries a hand-written "why / when / reading order" section plus a
# GENERATED table of its topics. The table is generated for the same reason COVERAGE.md is:
# a hand-maintained topic list drifts the moment a topic is added, and a navigation layer
# that lies is worse than none. Content between the markers is owned by this script;
# everything outside them is owned by a human and is never touched.
$BEGIN = '<!-- BEGIN MEASURED -->'
$END   = '<!-- END MEASURED -->'

function Build-DomainBlock {
    param([string]$Domain, [object[]]$DomainRows)
    $b = [System.Text.StringBuilder]::new()
    $null = $b.AppendLine($BEGIN)
    $null = $b.AppendLine('<!-- Generated by tools/Build-CoverageRegister.ps1. Do not edit by hand. -->')
    $null = $b.AppendLine()
    $d = @($DomainRows | Where-Object Depth -eq 'DEEP').Count
    $w = @($DomainRows | Where-Object State -eq 'WRITTEN').Count
    $null = $b.AppendLine("**$($DomainRows.Count) topics · $d written to standard · $w carrying evidence.**")
    $null = $b.AppendLine()
    $null = $b.AppendLine('| Topic | Depth | KB | Evidence |')
    $null = $b.AppendLine('|---|---|---:|---:|')
    foreach ($r in ($DomainRows | Sort-Object Topic)) {
        $name = ($r.Topic -split '/')[-1]
        $null = $b.AppendLine("| [``$name``]($name/) | $($r.Depth) | $($r.KB) | $($r.FacetN)/6 |")
    }
    $null = $b.AppendLine()
    $null = $b.AppendLine('Depth measures whether the README teaches the concept; evidence counts the six facet')
    $null = $b.AppendLine('folders. They are independent - see [`../COVERAGE.md`](../COVERAGE.md).')
    $null = $b.Append($END)
    $b.ToString()
}

$domainStale = @()
foreach ($grp in ($rows | Group-Object { ($_.Topic -split '/')[0] })) {
    $readme = Join-Path (Join-Path $repo $grp.Name) 'README.md'
    if (-not (Test-Path $readme)) { continue }
    $raw = Get-Content $readme -Raw
    if ($raw -notmatch [regex]::Escape($BEGIN)) { continue }   # no markers: human-owned, leave alone
    $block = Build-DomainBlock -Domain $grp.Name -DomainRows $grp.Group
    $new = [regex]::Replace($raw,
        "$([regex]::Escape($BEGIN)).*?$([regex]::Escape($END))", { $block }, 'Singleline')
    $norm = { param($s) ($s -replace "`r`n", "`n").TrimEnd() }
    if ((& $norm $new) -ne (& $norm $raw)) {
        if ($Check) { $domainStale += $grp.Name }
        else {
            [System.IO.File]::WriteAllText($readme, ($new -replace "`r`n", "`n"),
                (New-Object System.Text.UTF8Encoding $false))
        }
    }
}

if ($Check -and $domainStale.Count) {
    Write-Host "Domain README(s) STALE: $($domainStale -join ', ')" -ForegroundColor Red
    Write-Host 'Run tools/Build-CoverageRegister.ps1' -ForegroundColor Red
    exit 1
}
